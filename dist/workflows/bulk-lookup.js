 ...
 */
export function parseBulkLookupArgs(rawArgs) {
    return {
        applications: rawArgs['applications'] || rawArgs['apps'] || rawArgs['list'] || '',
        output: (rawArgs['output'] || rawArgs['format'] || 'markdown'),
 ...
 */
function parseApplicationList(applications) {
    return applications
        .split(/[,;]/)
        .map(app => app.trim())
 ...
 */
async function lookupApplication(query, includeVersions, includeInstallers) {
 ...
 */
function formatAsCsv(results, includeInstallers) {
    ...
    for (const result of results) {
        ...
        if (includeInstallers) {
            values.push(escapeCSV(result.package?.installerType || ''), escapeCSV(result.package?.productCode || ''), escapeCSV(result.package?.silentArgs || ''));
        }
 ...
 */
function formatAsMarkdown(results, includeInstallers) {
    const lines = [];
    lines.push('| Query | Status | Package ID | Name | Publisher | Version |');
    lines.push('|-------|--------|------------|------|-----------|---------|');
    for (const result of results) {
        const status = result.found ? '✓' : '✗';
        lines.push(`| ${escapeMarkdown(result.query)} | ${status} | ${escapeMarkdown(result.package?.packageId || '-')} | ${escapeMarkdown(result.package?.packageName || '-')} | ${escapeMarkdown(result.package?.publisher || '-')} | ${escapeMarkdown(result.package?.version || '-')} |`);
    }
    if (includeInstallers) {
        lines.push('');
        ...
        for (const result of results) {
            if (result.found && result.package) {
                lines.push(`| ${escapeMarkdown(result.package.packageId)} | ${escapeMarkdown(result.package.installerType || '-')} | ${escapeMarkdown(result.package.productCode || '-')} | ${escapeMarkdown(result.package.silentArgs || '-')} |`);
            }
        }
    }
 ...
 */
export async function executeBulkLookupWorkflow(args) {
    ...
    for (let i = 0; i < applications.length; i += CONCURRENCY_LIMIT) {
        ...
        const batchResults = await Promise.all(batch.map(app => lookupApplication(app, args.includeVersions || false, args.includeInstallers || false)));
 ...
 */
export function formatBulkLookupResult(result) {
    const lines = [];
    // Header
...
//# sourceMappingURL=bulk-lookup.js.map