 ...
 */
export function parsePackageAppArgs(rawArgs) {
    const args = {
        ...
        architecture: (rawArgs['architecture'] || rawArgs['arch']),
        complexity: (rawArgs['complexity'] || 'standard'),
        environment: (rawArgs['environment'] || 'intune'),
    };
    return args;
}
/**
 ...
 */
async function lookupWinget(applicationName) {
 ...
 */
async function generateScript(lookup, complexity) {
 ...
 */
async function generateDetection(lookup) {
    if (!lookup.found) {
        return undefined;
    }
    ...
    try {
        ...
        const recommendation = detectionService.recommendDetectionType(lookup.installerType, !!lookup.productCode, true // Assume version file available for exe
        );
        let detectionResult;
        ...
        return {
            type: detectionResult.configuration.type,
            config: detectionResult.configuration.details,
            powershellScript: detectionResult.powershellScript,
            intuneJson: detectionResult.intuneJson,
        };
    }
    catch (error) {
 ...
 */
async function validateScript(script, environment) {
 ...
 */
export async function executePackageAppWorkflow(args) {
 ...
 */
function generatePackageStructureDoc(lookup) {
...
function generateNextSteps(result) {
    ...
    if (result.status === 'success') {
        steps.push('Download the installer file and place it in the Files directory');
        steps.push('Review the generated Deploy-Application.ps1 script');
 ...
 */
export function formatPackageAppResult(result) {
    const lines = [];
    // Header
    ...
    if (result.script) {
        ...
        lines.push('- Deploy-Application.ps1 (PSADT deployment script)');
        if (result.detection) {
