 ...
 */
export function parseConvertLegacyArgs(rawArgs) {
    return {
...
function detectVersion(script) {
    ...
    if (v4Score >= 2 && v4Score > v3Score)
        return 'v4';
    if (v3Score >= 2 && v3Score > v4Score)
        return 'v3';
    if (v3Score > 0)
        return 'v3';
    return 'unknown';
 ...
 */
function findFunctionMappings(script) {
    const mappings = [];
 ...
 */
function findVariableMappings(script) {
    const mappings = [];
    const lines = script.split('\n');
 ...
 */
function findStructureIssues(script) {
    const issues = [];
 ...
 */
function convertScript(script, functionMappings, variableMappings) {
 ...
 */
function identifyManualReviewPoints(originalScript, convertedScript, functionMappings, variableMappings) {
    const points = [];
 ...
 */
function generateChecklist(functionMappings, variableMappings, structureIssues) {
    const checklist = [];
    // Structure items
    checklist.push({
 ...
 */
export async function executeConvertLegacyWorkflow(args) {
    ...
    // Convert script
    result.convertedScript = convertScript(script, result.analysis.functionMappings, result.analysis.variableMappings);
    // Identify manual review points
    result.manualReviewPoints = identifyManualReviewPoints(script, result.convertedScript, result.analysis.functionMappings, result.analysis.variableMappings);
    ...
    // Generate checklist
    result.checklist = generateChecklist(result.analysis.functionMappings, result.analysis.variableMappings, result.analysis.structureIssues);
    // Set status
 ...
 */
export function formatConvertLegacyResult(result) {
    const lines = [];
    // Header
...
//# sourceMappingURL=convert-legacy.js.map