...
describe('DetectionService', () => {
    ...
    describe('generateMsiDetection', () => {
        ...
        it('should uppercase product code', () => {
            ...
            const result = service.generateMsiDetection(input);
            const msiJson = result.intuneJson;
            expect(msiJson.productCode).toBe('{ABCDEF12-ABCD-ABCD-ABCD-ABCDEF123456}');
        });
        it('should reject invalid product code format', () => {
            const input = {
                productCode: 'not-a-guid',
            };
            expect(() => service.generateMsiDetection(input)).toThrow(/Invalid MSI product code format/);
        });
    ...
    describe('Intune JSON schema compliance', () => {
        it('should include correct @odata.type for file detection', () => {
            ...
            expect(result.intuneJson?.['@odata.type']).toBe('#microsoft.graph.win32LobAppFileSystemDetection');
        });
        it('should include correct @odata.type for registry detection', () => {
            ...
            expect(result.intuneJson?.['@odata.type']).toBe('#microsoft.graph.win32LobAppRegistryDetection');
        });
        it('should include correct @odata.type for MSI detection', () => {
            const result = service.generateMsiDetection({
                productCode: '{12345678-1234-1234-1234-123456789012}',
            });
            expect(result.intuneJson?.['@odata.type']).toBe('#microsoft.graph.win32LobAppProductCodeDetection');
        });
    });
