...
const EXIT_CODE_DATABASE: Map<number, ExitCodeInfo> = new Map([
  // MSI Success codes
 ...
 */
export function parseTroubleshootArgs(rawArgs: Record<string, string>): TroubleshootArguments {
  ...
  return {
    ...
    installerType: installerTypeValue as InstallerType | undefined,
 ...
 */
function lookupExitCode(code: number, installerType?: InstallerType): ExitCodeInfo | undefined {
 ...
 */
function analyzeLogFile(logPath: string): LogAnalysis | undefined {
  ...
  try {
    ...
    const errorEntries: LogAnalysis['errorEntries'] = [];
 ...
 */
function identifyCauses(
  args: TroubleshootArguments,
  exitCodeInfo?: ExitCodeInfo,
  logAnalysis?: LogAnalysis
): IdentifiedCause[] {
  const causes: IdentifiedCause[] = [];
  ...
  if (logAnalysis) {
    const patternCauses = new Map<string, { count: number; evidence: string[] }>();
...
function getRelevantResources(
  causes: IdentifiedCause[],
  exitCodeInfo?: ExitCodeInfo
): Array<{ title: string; uri: string; description: string }> {
  const resources: Array<{ title: string; uri: string; description: string }> = [];
  ...
  const mentionsDetection = causes.some(c =>
    c.description.toLowerCase().includes('detection')
  );
 ...
 */
export async function executeTroubleshootWorkflow(
  args: TroubleshootArguments
 ...
 */
export function formatTroubleshootResult(result: TroubleshootResult): string {
  const lines: string[] = [];
