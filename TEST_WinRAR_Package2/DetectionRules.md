...
This document provides detection rules for WinRAR 7.13.0 deployment via Microsoft Intune. Two detection methods are provided - choose the one that best fits your environment.
...
Detects WinRAR by checking the uninstall registry key.
...
- **File detection** is recommended as it directly verifies the application binary exists and is the correct version
- **Registry detection** is useful as a fallback or when file paths may vary
- For upgrades, version comparison ensures older versions are detected for update
- Test detection rules in a lab environment before production deployment
